...
namespace DarkDemo
{
    public partial class WeeklyReportDetail : Form
    {
        ...
        public WeeklyReportDetail(Staff staff, DateTime sDate, DateTime eDate)
        {
            InitializeComponent();
            this.staff = staff;
            this.sDate = sDate;
        ...
        {
            report = db.GetWeeklyNumIncome(sDate, eDate);
            lblNoOfOrder.Text += " " + report[0].totalNumber.ToString();
            lblIncome.Text += " " + report[0].totalIncome.ToString();
            lblReportID.Text += " " + sDate.ToString("yyyyMMdd");
            lblReportDate.Text += "\n" + sDate.ToString("yyyy-MM-dd") + " to " + eDate.ToString("yyyy-MM-dd");
            ...
            for (int i = 0; i < 4; i++)
            {
                from[i] = db.GetWeeklyFromLocation(sDate, eDate, Location[i])[0].totalNumber;
            }
            for (int i = 0; i < 4; i++)
            {
            ...
            for (int i = 0; i < 4; i++)
            {
                to[i] = db.GetWeeklyToLocation(sDate, eDate, shortForm(Location[i]))[0].totalNumber;
            }
            for (int i = 0; i < 4; i++)
            {
